...
const SliceQuantGradingMRBasicUtil = {
  calculateScores(answer, selectedAnswer, modifiers) {
    //console.log('++++++++++++++++++++++++++++++ Slice Quant score ++++++++++++++++++++++')
    let scoreDetails = {}
    scoreDetails.slicePrescription = this.slicePrescriptionScore(answer, selectedAnswer, modifiers)
    scoreDetails.combinedScore = _.round(scoreDetails.slicePrescription.combinedScoreAvg * 1.0, 2)
  ...
  slicePrescriptionScore(answer, selectedAnswer, modifiers) {
    ...
    overrides.factors = {
      ...
      spacing: {
        ignore: selectedAnswer.isSingleSlice,
        directional: true,
        scoringTooHigh: {
          linearFactor: 100,
          //quadraticBuffer: 0.2,
    ...
    const slicePrescription = {}
    slicePrescription.sliceGroups = []
    let scoreTotal = 0
    for (var scoreVars of scores.groupScoreVariables) {
      //console.log('======================scoreVars===================')
      //console.log(scoreVars)
      const group = {}
      group.scores = {}
      scoreVars.analysis.forEach((x) => {
        group.scores[x.factorName] = _.round(100 - x.scoreLoss, 2)
      })

      let combinedLoss = 0
      if (selectedAnswer.isSingleSlice) {
        combinedLoss =
          (100 - group.scores.angle) * 0.3 +
          (100 - group.scores.inplaneRotationAngle) * 0.5 +
          (100 - group.scores.thickness) * 0.2 +
          (100 - group.scores.dimensionX) * 0.2 +
          (100 - group.scores.dimensionY) * 0.2 +
          (100 - group.scores.singleSliceCoverageZ) * 0.3
      } else {
        combinedLoss =
          (100 - group.scores.angle) * 0.3 +
          (100 - group.scores.inplaneRotationAngle) * 0.5 +
          (100 - group.scores.thickness) * 0.2 +
          (100 - group.scores.spacing) * 0.11 +
          (100 - group.scores.dimensionX) * 0.2 +
          (100 - group.scores.dimensionY) * 0.2 +
          (100 - group.scores.coverageX) * 0.2 +
          (100 - group.scores.coverageY) * 0.2 +
          (100 - group.scores.coverageZ) * 0.3
      }
      group.combinedScore = _.round(_.max([0, 100 - combinedLoss]), 2)
      group.groupScoreVariables = scoreVars
      group.rubric = scores.rubric
      slicePrescription.sliceGroups.push(group)
      scoreTotal += group.combinedScore
    }
    slicePrescription.combinedScoreAvg = _.round(scoreTotal / slicePrescription.sliceGroups.length, 2)
    /* new code ends here */

    // original code. only here for ref. Can delete when new code is in place
    /*
    const slicePrescriptionScore = {}
    scores.groupScoreVariables[0].analysis.forEach((x) => {
      slicePrescriptionScore[x.factorName] = 100 - x.scoreLoss
    })

    const combinedScoreLoss =
...
module.exports = SliceQuantGradingMRBasicUtil
