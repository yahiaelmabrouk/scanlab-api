...
no_exams <-  dbGetQuery(con, 'SELECT
                              DISTINCT "cohortId"
                  ...
                  LEFT JOIN "Cohorts" ON "CohortStudents"."cohortId"="Cohorts"."id" WHERE "PreparedExams"."title" IS NULL')

with_exams <-  dbGetQuery(con, 'SELECT
                              DISTINCT "cohortId"
                  FROM "MultipleChoiceQuestionResults"
                  LEFT JOIN "Users" ON "MultipleChoiceQuestionResults"."userId"="Users"."id"
                  LEFT JOIN "MultipleChoiceQuestions" ON "MultipleChoiceQuestionResults"."multipleChoiceQuestionId"="MultipleChoiceQuestions"."id"
                  ...
                  LEFT JOIN "PreparedExams" ON "TestRuns"."preparedExamId"="PreparedExams"."id"
                  LEFT JOIN "CohortStudents" ON "Users"."id"="CohortStudents"."userId"
                  LEFT JOIN "Cohorts" ON "CohortStudents"."cohortId"="Cohorts"."id" WHERE "PreparedExams"."title" IS NOT NULL')
...
getQuartiles <- function(persons) {
  for(i in 1:nrow(persons)){
    persons[i, 'quartile'] <- (1 - round(100/nrow(persons)*i)/100) * 100
  }
  return(persons)
}

persons <- getQuartiles(persons = persons)
persons

plot3 <- ggplot(persons,aes(x=Person,y=Rating,
                            ymin=Rating-Deviation, ymax=Rating+Deviation,
                            fill = Volatility,  labelc=quartile, color=cohort)) +
  scale_fill_viridis_c() +
  geom_crossbar(fatten=0, linetype = 0, na.rm = TRUE) +
  geom_point( persons[complete.cases(persons$cohort), ], mapping=aes(x=Person,y=Rating,
                                                                     color=cohort, label=legalName,), na.rm=TRUE, size=2, fill=persons[complete.cases(persons$cohort), ]$Volatility, show.legend=TRUE, alpha=1)+
  # geom_point( persons, mapping=aes(x=Person,y=Rating,label=quartile,), na.rm=TRUE, size=0, fill=persons$Volatility, show.legend=TRUE, alpha=1)+
  geom_vline(xintercept = nrow(persons)* 1 /5, color='blue') +
  geom_vline(xintercept = nrow(persons)* 2 /5, color='blue') +
  geom_vline(xintercept = nrow(persons)* 3 /5, color='blue') +
  geom_vline(xintercept = nrow(persons)* 4 /5, color='blue') +
  ggtitle("Conceptual knowledge rating") +
  theme(axis.text.x = element_text(angle = 90,vjust=0.25))
plot3_plotly <- ggplotly(plot3)
plot3_plotly



# htmlwidgets::saveWidget(
#   widget = plot3_plotly, #the plotly object
#   file = "public/plot3.html", #the path & file name
#   selfcontained = TRUE #creates a single html file
# )

htmlwidgets::saveWidget(widget = as_widget(plot3_plotly), file = "plot3.html", selfcontained = TRUE)
file.rename("plot3.html", "public/plot3.html")
##################################################################
# Practical questions
#
# prepare data for Glicko
...
plot5 <- ggplot(persons,aes(x=Person,y=Rating,
                            ymin=Rating-Deviation, ymax=Rating+Deviation,
                            fill = Volatility, color = cohort,  labelc=quartile)) +
  scale_fill_viridis_c() +
  geom_crossbar(fatten=0, linetype = 0, na.rm = TRUE) +
  geom_point( persons[complete.cases(persons$cohort), ], mapping=aes(x=Person,y=Rating,
                                                                     color=cohort, label=legalName,), na.rm=TRUE, size=2, fill=persons[complete.cases(persons$cohort), ]$Volatility, show.legend=TRUE, alpha=1)+
  ...
  geom_vline(xintercept = nrow(persons)* 3 /5, color='blue') +
  geom_vline(xintercept = nrow(persons)* 4 /5, color='blue') +
  ggtitle("Total skill rating") +
  theme(axis.text.x = element_text(angle = 90,vjust=0.25))

plot5_plotly <- ggplotly(plot5)
plot5_plotly

# htmlwidgets::saveWidget(
#   widget = plot5_plotly, #the plotly object
#   file = "public/plot5.html", #the path & file name
#   selfcontained = TRUE #creates a single html file
# )

htmlwidgets::saveWidget(widget = as_widget(plot5_plotly), file = "plot5.html", selfcontained = TRUE)
file.rename("plot5.html", "public/plot5.html")
...
plot6 <- ggplot(persons,aes(x=Person, y=Rating_Brain,
                            ymin=Rating_Brain-Deviation_Brain,
                            ymax=Rating_Brain+Deviation_Brain,
                            fill = Volatility_Brain, color = cohort, labelc=quartile)) +
  scale_fill_viridis_c() +
  geom_crossbar(fatten=0, linetype = 0, na.rm = TRUE) +
  geom_point( persons[complete.cases(persons$cohort), ], mapping=aes(x=Person,y=Rating_Brain,
                                                                     color=cohort,label=legalName), na.rm=TRUE, size=2, fill=persons[complete.cases(persons$cohort), ]$Volatility_Brain, show.legend=TRUE, alpha=1)+
  # geom_point( persons, mapping=aes(x=Person,y=Rating_Brain,label=quartile,), na.rm=TRUE, size=0, fill=persons$Volatility_Brain, show.legend=TRUE, alpha=1)+
  geom_vline(xintercept = nrow(persons)* 1 /5, color='blue') +
  geom_vline(xintercept = nrow(persons)* 2 /5, color='blue') +
  geom_vline(xintercept = nrow(persons)* 3 /5, color='blue') +
  geom_vline(xintercept = nrow(persons)* 4 /5, color='blue') +
  ggtitle("Brain skill rating") +
  theme(axis.text.x = element_text(angle = 90,vjust=0.25))
plot6_plotly <- ggplotly(plot6)
plot6_plotly


# htmlwidgets::saveWidget(
#   widget = plot6_plotly, #the plotly object
#   file = "public/plot6.html", #the path & file name
#   selfcontained = TRUE #creates a single html file
# )

htmlwidgets::saveWidget(widget = as_widget(plot6_plotly), file = "plot6.html", selfcontained = TRUE)
file.rename("plot6.html", "public/plot6.html")

plot7 <- ggplot(persons,aes(x=Person, y=Rating_Knee,
                            ymin=Rating_Knee-Deviation_Knee,
                            ymax=Rating_Knee+Deviation_Knee,
                            fill = Volatility_Knee, color = cohort, labelc=quartile)) +
  scale_fill_viridis_c() +
  geom_crossbar(fatten=0, linetype = 0, na.rm = TRUE) +
  geom_point( persons[complete.cases(persons$cohort), ], mapping=aes(x=Person,y=Rating_Knee,
                                                                     color=cohort,label=legalName), na.rm=TRUE, size=2, fill=persons[complete.cases(persons$cohort), ]$Volatility_Knee, show.legend=TRUE, alpha=1)+
  # geom_point( persons, mapping=aes(x=Person,y=Rating_Knee,label=quartile,), na.rm=TRUE, size=0, fill=persons$Volatility_Knee, show.legend=TRUE, alpha=1)+
  geom_vline(xintercept = nrow(persons)* 1 /5, color='blue') +
  geom_vline(xintercept = nrow(persons)* 2 /5, color='blue') +
  geom_vline(xintercept = nrow(persons)* 3 /5, color='blue') +
  geom_vline(xintercept = nrow(persons)* 4 /5, color='blue') +
  ggtitle("Knee skill rating") +
  theme(axis.text.x = element_text(angle = 90,vjust=0.25))
plot7_plotly <- ggplotly(plot7)
plot7_plotly


# htmlwidgets::saveWidget(
#   widget = plot7_plotly, #the plotly object
#   file = "public/plot7.html", #the path & file name
#   selfcontained = TRUE #creates a single html file
# )

htmlwidgets::saveWidget(widget = as_widget(plot7_plotly), file = "plot7.html", selfcontained = TRUE)
file.rename("plot7.html", "public/plot7.html")

plot8 <- ggplot(persons,aes(x=Person, y=Rating_Lumbar,
                            ymin=Rating_Lumbar-Deviation_Lumbar,
                            ymax=Rating_Lumbar+Deviation_Lumbar,
                            fill = Volatility_Lumbar, color = cohort, labelc=quartile)) +
  scale_fill_viridis_c() +
  geom_crossbar(fatten=0, linetype = 0, na.rm = TRUE) +
  geom_point( persons[complete.cases(persons$cohort), ], mapping=aes(x=Person,y=Rating_Lumbar,
                                                                     color=cohort,label=legalName), na.rm=TRUE, size=2, fill=persons[complete.cases(persons$cohort), ]$Volatility_Lumbar, show.legend=TRUE, alpha=1)+
  # geom_point( persons, mapping=aes(x=Person,y=Rating_Lumbar,label=quartile,), na.rm=TRUE, size=0, fill=persons$Volatility_Lumbar, show.legend=TRUE, alpha=1)+
  geom_vline(xintercept = nrow(persons)* 1 /5, color='blue') +
  geom_vline(xintercept = nrow(persons)* 2 /5, color='blue') +
  geom_vline(xintercept = nrow(persons)* 3 /5, color='blue') +
  geom_vline(xintercept = nrow(persons)* 4 /5, color='blue') +
  ggtitle("Lumbar skill rating") +
  theme(axis.text.x = element_text(angle = 90,vjust=0.25))
plot8_plotly <- ggplotly(plot8)
plot8_plotly


# htmlwidgets::saveWidget(
#   widget = plot8_plotly, #the plotly object
#   file = "public/plot8.html", #the path & file name
#   selfcontained = TRUE #creates a single html file
# )

htmlwidgets::saveWidget(widget = as_widget(plot8_plotly), file = "plot8.html", selfcontained = TRUE)
file.rename("plot8.html", "public/plot8.html")
